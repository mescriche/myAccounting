...
class RepoView(ttk.Frame):
    ...
    def create_gui(self):
        ...
        self.income = IncomeRepoView(self.notebook, self.user, self.acc_tree)
        self.notebook.add(self.income, text='Income')

        self.tax = TaxRepoView(self.notebook, self.user, self.acc_tree)
        self.notebook.add(self.tax, text='Tax')

        self.insurance = InsRepoView(self.notebook, self.user, self.acc_tree)
        self.notebook.add(self.insurance, text='Insurance')

        self.expenses = ExpRepoView(self.notebook, self.user, self.acc_tree)
        self.notebook.add(self.expenses, text='Expense')
        
        self.balance = BalanceRepoView(self.notebook, self.user, self.acc_tree)
        self.notebook.add(self.balance, text='Balance')

        self.summary = SummaryRepoView(self.notebook, self.user, self.acc_tree)
